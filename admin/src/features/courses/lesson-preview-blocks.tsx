...
import { Check, ChevronDown, Copy, FileText, Info, Lightbulb, Maximize2, Minimize2, PanelRightClose, PanelRightOpen, Paperclip, RotateCcw, Sparkles, TriangleAlert } from 'lucide-react'
import {
...
function PlaygroundFilePreview({ url, label }: { url: string; label: string }) {
  ...
  if (isTextFile) {
    ...
    if (isMarkdown) {
      return <article className="size-full min-h-80 overflow-auto bg-white px-6 py-5 text-[15px] leading-7 text-zinc-900 [&_a]:text-orange-600 [&_a]:underline [&_blockquote]:my-4 [&_blockquote]:border-l-2 [&_blockquote]:border-orange-300 [&_blockquote]:pl-4 [&_code]:rounded [&_code]:bg-zinc-100 [&_code]:px-1.5 [&_code]:py-0.5 [&_h1]:mb-4 [&_h1]:text-2xl [&_h1]:font-bold [&_h2]:mb-3 [&_h2]:mt-7 [&_h2]:text-xl [&_h2]:font-bold [&_h3]:mb-2 [&_h3]:mt-5 [&_h3]:text-lg [&_h3]:font-semibold [&_hr]:my-6 [&_li]:ml-5 [&_li]:pl-1 [&_ol]:my-4 [&_ol]:list-decimal [&_p]:my-3 [&_pre]:my-4 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-zinc-100 [&_pre]:p-4 [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_table]:my-4 [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:border-zinc-200 [&_td]:px-3 [&_td]:py-2 [&_th]:border [&_th]:border-zinc-200 [&_th]:bg-zinc-100 [&_th]:px-3 [&_th]:py-2 [&_ul]:my-4 [&_ul]:list-disc"><Markdown remarkPlugins={[remarkGfm]}>{textContent}</Markdown></article>
    }
...
function PlaygroundChatAnswer({ answer }: { answer: string }) {
  return <div className="overflow-x-auto text-[15px] leading-7 text-zinc-900 [&_a]:text-orange-600 [&_a]:underline [&_code]:rounded [&_code]:bg-zinc-100 [&_code]:px-1.5 [&_code]:py-0.5 [&_h1]:mb-3 [&_h1]:text-xl [&_h1]:font-bold [&_h2]:mb-2 [&_h2]:mt-5 [&_h2]:text-lg [&_h2]:font-bold [&_h3]:mb-2 [&_h3]:mt-4 [&_h3]:font-semibold [&_li]:ml-5 [&_ol]:my-3 [&_ol]:list-decimal [&_p]:my-2 [&_pre]:my-3 [&_pre]:overflow-x-auto [&_pre]:rounded-xl [&_pre]:bg-zinc-100 [&_pre]:p-3 [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_table]:my-3 [&_table]:w-full [&_table]:border-collapse [&_td]:border [&_td]:border-zinc-200 [&_td]:px-3 [&_td]:py-2 [&_th]:border [&_th]:border-zinc-200 [&_th]:bg-zinc-100 [&_th]:px-3 [&_th]:py-2 [&_ul]:my-3 [&_ul]:list-disc"><Markdown remarkPlugins={[remarkGfm]}>{answer}</Markdown></div>
}
      ...
      <div className="flex min-h-0 flex-col border-b border-zinc-200 lg:border-r lg:border-b-0">
        ...
        <div className={cn('min-h-80 flex-1 overflow-y-auto px-4 py-5', tab === 'chat' && 'bg-zinc-50')}>
          {tab === 'prompt' ? <pre className="mx-auto max-w-2xl whitespace-pre-wrap font-mono text-[14px] leading-7 text-zinc-900">{block.prompt}</pre> : <div className="mx-auto flex max-w-2xl flex-col gap-5">
            <div className="flex justify-end"><div className="flex max-w-[85%] flex-col items-end gap-2"><div className="w-full rounded-xl rounded-br-sm bg-zinc-100 p-3 shadow-sm"><div className="relative"><pre className={cn('whitespace-pre-wrap font-mono text-[14px] leading-7 text-zinc-900', !promptExpanded && 'line-clamp-6')}>{block.prompt}</pre>{!promptExpanded && block.prompt.length > 220 ? <span className="pointer-events-none absolute inset-x-0 bottom-0 h-14 bg-gradient-to-b from-transparent to-zinc-100" /> : null}</div>{block.prompt.length > 220 ? <button type="button" onClick={() => setPromptExpanded((value) => !value)} className="mt-2 flex items-center gap-1.5 rounded-full bg-white px-3 py-1 text-xs font-semibold text-zinc-800">{promptExpanded ? 'Show less' : 'Show more'}<ChevronDown className={cn('size-3.5 transition-transform', promptExpanded && 'rotate-180')} /></button> : null}</div>{block.documentUrl ? <div className="flex h-[58px] w-full min-w-64 items-center gap-3 rounded-xl border border-zinc-200 bg-white p-3 text-zinc-900 shadow-sm"><span className="flex size-9 items-center justify-center rounded-lg bg-orange-50 text-orange-600"><FileText className="size-5" /></span><span className="min-w-0 flex-1 truncate text-sm font-semibold">{block.documentLabel || 'Input document'}</span></div> : null}</div></div>
            {generating ? <div className="flex items-center gap-3 py-2 text-sm text-zinc-500" role="status" aria-live="polite"><span className="flex items-center gap-1"><span className="size-2 animate-bounce rounded-full bg-orange-500 [animation-delay:-0.3s]" /><span className="size-2 animate-bounce rounded-full bg-orange-500 [animation-delay:-0.15s]" /><span className="size-2 animate-bounce rounded-full bg-orange-500" /></span><span>Claude is working…</span></div> : <div className="flex flex-col gap-3"><div className="flex items-start gap-2.5"><span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-full bg-orange-100 text-orange-600"><Sparkles className="size-4" /></span><div className="max-w-[85%] rounded-2xl rounded-tl-sm border border-zinc-200 bg-white px-4 py-3 shadow-sm"><PlaygroundChatAnswer answer={block.answer} /></div></div>{block.previewUrl ? <button type="button" onClick={() => setPreviewOpen(true)} className="flex h-[66px] items-center gap-3 rounded-xl border border-zinc-200 bg-white p-3 text-left text-zinc-900 shadow-sm transition-colors hover:border-orange-300"><span className="flex size-10 items-center justify-center rounded-lg bg-orange-50 text-orange-600"><FileText className="size-5" /></span><span className="min-w-0 flex-1 truncate text-sm font-semibold">{block.previewLabel || 'Generated output'}</span><span className="rounded-full bg-zinc-100 px-3 py-1.5 text-xs font-semibold">View</span></button> : null}</div>}
          </div>}
        </div>
