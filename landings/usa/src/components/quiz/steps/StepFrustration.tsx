...
export default function StepFrustration() {
  const { answers, setAnswer, nextStep } = useQuiz();
  const value = answers.frustration || 0;
    ...
    <div>
      ...
      <div className="mb-9 rounded-xl px-5 py-5" style={{ background: '#F7F7F7', border: '2px solid #E0E0E0' }}>
        <p className="text-[17px] italic leading-relaxed text-center" style={{ color: '#333' }}>
          "I know I'm capable of more, but I don't know where to start – and that frustrates me"
        </p>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 mb-3">
        {[1, 2, 3, 4, 5].map((n) => {
          ...
          return (
            <button
              key={n}
              onClick={() => setAnswer("frustration", n)}
